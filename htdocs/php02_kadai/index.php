      ...
      if ($("#keyword").val() == "") {
        ...
        axios.get(url).then(function(res) {
          ...
          items.forEach(function(item, index) {
            ...
            var publisher = item.volumeInfo.publisher || '出版社情報なし'; // 出版社がundefinedの場合のデフォルトテキスト

            if ($("#content:contains('" + item.volumeInfo.title + "')").length == 0) {
              $("#content").append(
                `<div class="book-container" data-book-id="${index}">
                 <img src='${item.volumeInfo.imageLinks.thumbnail}' class="book-thumbnail"/>
                 <div class="book-details">
                   <h2 class="book-title">${item.volumeInfo.title}</h2>
                   <p class="book-author">著者: ${authors.join(', ')}</p>
                   <p class="book-publisher">出版社: ${item.volumeInfo.publisher}</p>
